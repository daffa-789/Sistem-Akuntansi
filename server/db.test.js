...
describe('SQLite Database Layer', () => {
  it('should query accounts from sqlite', async () => {
    const [accounts] = await pool.query('SELECT * FROM accounts WHERE company_id = 1')
  ...
  it('should normalize boolean and handle transactions', async () => {
    ...
    const [rows] = await pool.query('SELECT * FROM audit_logs WHERE id = ?', [result])
  ...
  it('should rollback transaction on error', async () => {
    ...
    const [rows] = await pool.query("SELECT * FROM audit_logs WHERE action = 'ROLLBACK_ME'")
