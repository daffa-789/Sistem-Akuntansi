...
import { Account, Company, JournalEntry, JournalLine } from '../../shared/types.js'
...
function autoFitColumns(ws: ExcelJS.Worksheet, minWidths: Record<number, number> = {}): void {
...
function saveWorkbook(workbook: ExcelJS.Workbook, filename: string): Promise<void> {
...
export interface ExportJournalParams {
  company?: Company | null
  entries?: (JournalEntry & { lines?: (JournalLine & { code?: string; account_name?: string })[] })[]
 ...
 */
export async function exportLedgerToExcel({ company, account, rows = [], initialBalance = 0, range }: ExportLedgerParams): Promise<void> {
  const wb = new ExcelJS.Workbook()
  wb.creator = 'Finova Akuntansi Indonesia'
...
export interface ExportTrialBalanceParams {
  company?: Company | null
  accounts?: any[]
  asOfDate?: string
  totals?: { debit?: number; credit?: number }
 ...
 */
export async function exportTrialBalanceToExcel({ company, accounts = [], asOfDate, totals }: ExportTrialBalanceParams): Promise<void> {
  const wb = new ExcelJS.Workbook()
  wb.creator = 'Finova Akuntansi Indonesia'
 ...
 */
export async function exportAccountsToExcel({ company, accounts = [] }: ExportAccountsParams): Promise<void> {
  const wb = new ExcelJS.Workbook()
  wb.creator = 'Finova Akuntansi Indonesia'
